...
**팀원 고정 목록**: 이 프로젝트 개발자는 `pbk95120`, `jongse7` 단 둘이다. `$1`이 없으면
GitHub API 검색 없이 이 목록에서 바로 정한다 — 리뷰어를 물어보지 말 것.

0. `$1`이 **없는** 경우 (우선 시도):
   - `ME`(현재 로그인, Step 0)가 `pbk95120`이면 리뷰어 = `jongse7`, `ME`가 `jongse7`이면 리뷰어 = `pbk95120`.
   - `ME`가 이 둘 중 아무것도 아니면(신규 팀원 추가 등) 자동 지정을 포기하고 아래 1~3번 절차로 폴백.
   - 자동 지정했으면 1~3번은 건너뛰고 바로 4번으로.
1. `$1`이 **있는** 경우(또는 0번이 폴백된 경우), 실제 GitHub 로그인으로 매칭. 순서대로, 첫 매칭 채택:
