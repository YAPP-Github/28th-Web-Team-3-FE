  ...
  return (
    <LegalPageLayout title="개인정보처리방침">
      ...
      <LegalSection heading="제6조 (AI를 활용한 미션 추천 및 자동화된 처리)">
        <LegalOrderedList
          items={[
            "운영팀은 이용자가 입력한 재무상태 응답(생년월일, 소득, 자산 등)을 Google LLC의 Gemini API에 전달하여 맞춤 미션 추천을 생성합니다.",
            "Gemini API에 전달되는 정보는 미션 추천 생성이라는 단일 목적을 위해서만 처리되며, 운영팀은 자체 AI 모델을 별도로 학습(파인튜닝 등)시키지 않습니다.",
            "Google LLC는 계약 및 약관에 따라 전달받은 정보를 자신의 모델 학습에 사용하지 않는 것을 원칙으로 하며, 관련 정책은 Google의 개인정보처리방침을 따릅니다.",
      ...
      <LegalSection heading="제7조 (개인정보의 국외 이전)">
        ...
        <LegalList
          items={[
            "제공받는 자: Google LLC (Gemini API)",
            "이전 국가: 미국",
            "이전 항목: 재무상태 관련 응답(생년월일, 소득, 자산 등 미션 추천에 필요한 최소한의 정보)",
            "이전 목적: AI 기반 맞춤 미션 추천 생성",
            "이전 방법: 서비스 이용 시점에 API를 통한 네트워크 전송",
            "보유 및 이용기간: Google의 정책에 따름 (원칙적으로 응답 생성 목적 달성 후 지체 없이 처리 종료)",
          ]}
        />
        <p>
          이전되는 개인정보 항목, 제공받는 자, 이전 국가 등은 AI 제공업체 변경 시 지체 없이 본
          방침을 통해 갱신합니다.
        </p>
      </LegalSection>
      ...
      <LegalSection heading="제9조 (개인정보의 제3자 제공)">
        <p>
          ...
          있거나 법령에 특별한 규정이 있는 경우에는 예외로 하며, 제6조·제7조에 따른 Google LLC로의
          처리 위탁은 본 조의 제3자 제공과 별개로 제6조·제7조를 따릅니다.
        </p>
      </LegalSection>
      ...
      <LegalSection heading="제10조 (정보주체의 권리·의무 및 행사방법)">
        <LegalOrderedList
          items={[
            ...
            "이용자는 앱 내 마이페이지의 '탈퇴하기'를 통해 직접 삭제를 실행할 수 있습니다. 탈퇴 시 게스트 계정과 서버에 저장된 개인정보(재무상태 응답, 목표, 미션 기록 등)가 즉시 삭제되며, 삭제된 정보는 복구되지 않습니다. 다만 제4조에 따라 관계 법령상 보존 의무가 있는 정보는 해당 기간 동안 보관합니다.",
            `그 밖의 권리 행사는 앱 내 마이페이지의 '문의하기(카카오톡 오픈채팅)' 또는 ${PRIVACY_CONTACT_EMAIL}로 요청하실 수 있으며, 운영팀은 요청 확인 후 지체 없이 필요한 조치를 취합니다.`,
          ]}
        />
      </LegalSection>

      <LegalSection heading="제11조 (만 16세 미만의 개인정보 처리 제한)">
        <p>
          운영팀의 서비스는 이용약관에 따라 만 16세 미만은 이용하실 수 없습니다. 만 16세 미만의
          개인정보가 수집된 사실이 확인될 경우, 운영팀은 해당 정보를 지체 없이 삭제합니다.
        </p>
      </LegalSection>
      ...
      <LegalSection heading="제12조 (개인정보의 안전성 확보조치)">
        ...
        <LegalOrderedList
          items={[
            ...
            "Gemini API 전달 시 미션 추천에 필요한 최소한의 정보만 전송",
          ]}
        />
      </LegalSection>
