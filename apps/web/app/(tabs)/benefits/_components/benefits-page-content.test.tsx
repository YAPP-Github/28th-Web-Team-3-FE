...
it("절약 팁을 선택할 때만 팁 히어로 문구를 보인다", () => {
  ...
  expect(screen.getByText("지금 바로 신청하기 좋은 혜택")).toBeInTheDocument();
  expect(screen.queryByText("지금 바로 챙기면 좋은 절약 팁")).not.toBeInTheDocument();

  fireEvent.click(screen.getByRole("button", { name: "policy" }));

  expect(screen.getByText("지금 바로 챙기면 좋은 절약 팁")).toBeInTheDocument();
  expect(screen.queryByText("지금 바로 신청하기 좋은 혜택")).not.toBeInTheDocument();
});
