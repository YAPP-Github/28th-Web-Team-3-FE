...
import { policy } from "@/lib/test/fixtures/policy";
...
describe("readCachedPolicyCategories", () => {
  it("카테고리별 무한 목록 캐시를 모두 훑는다", () => {
    ...
    queryClient.setQueryData(policiesOptions(null).queryKey, {
      pages: [[policy(1, { category: "금융" })], [policy(2, { category: "주거" })]],
      pageParams: [0, 1],
    });
    queryClient.setQueryData(policiesOptions("교육").queryKey, {
      pages: [[policy(3, { category: "교육" })]],
      pageParams: [0],
    });
  ...
  it("홈 캐러셀 캐시에서도 읽는다", () => {
    const queryClient = seedableQueryClient();
    queryClient.setQueryData(homePoliciesOptions().queryKey, [policy(9, { category: "복지" })]);
