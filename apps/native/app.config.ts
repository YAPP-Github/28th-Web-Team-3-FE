...
/**
 * 프로덕션 빌드에 WebView·API 주소가 빠지거나 잘못 들어가지 않았는지 막는다.
 *
 ...
 *
 * dev·preview 빌드는 검사하지 않는다 — 로컬 기본값(localhost)으로 도는 게 정상이다.
 */
function assertProductionEnv(): void {
  ...
  for (const key of ["EXPO_PUBLIC_WEB_URL", "EXPO_PUBLIC_API_URL"] as const) {
    ...
    if (!value) {
      throw new Error(
        `${key}가 없다. production 빌드에 필요하다. ` +
          "`cd apps/native && eas env:push production --path .env.production`로 반영해라.",
      );
    }
